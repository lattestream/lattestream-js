  ...
  try {
    ...
    if (trimmedText.startsWith('{') || trimmedText.startsWith('[')) {
      try {
        ...
        // Fall through to try binary protocol parsing
      }
    }
    
    // Check if this might be a binary protocol message
    if (bytes.length >= 8) {
      // Check if it has a valid binary protocol header
      ...
      // Valid message types and reasonable payload length
    ...
    // If it's not recognizable as either JSON or binary protocol,
    // but it's valid UTF-8 text, try parsing as JSON anyway
    if (trimmedText.length > 0) {
      try {
        return JSON.parse(trimmedText);
      } catch (finalJsonError) {
        console.log('Final JSON parse attempt failed:', finalJsonError);
        throw new Error(`Unable to parse binary message as JSON or binary protocol. Text: ${trimmedText.substring(0, 100)}...`);
      }
    }
...
export function parseLatteStreamBinaryProtocol(bytes: Uint8Array): any {
  ...
  console.log(`Binary protocol: messageType=0x${messageType.toString(16).padStart(2, '0')}, payloadLength=${payloadLength}, totalBytes=${bytes.length}`);
...
function parseBinaryPayloadWithMetadata(payloadBytes: Uint8Array): any {
  ...
  if (payloadBytes.length < 4 + metadataLength) {
    throw new Error(`Binary payload metadata truncated: expected ${4 + metadataLength} bytes, got ${payloadBytes.length}`);
  }
